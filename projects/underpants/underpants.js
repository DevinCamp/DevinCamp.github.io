...
_.unique = function(arr) {
    var elements = [];
    _.reject(arr, function(val, ind, coll) {
        if ( _.indexOf(elements, val) === -1) {
            elements.push(val);
        }
    });
    return elements;
...
_.every = (coll, test) => {
    ...
    let pass = true;
     _.each(coll, (item, i, coll) => {
        if(!test(item, i, coll)) pass = false;
    });
    return pass;
};
...
_.reduce = function(arr, func, seed) {
    ...
    /* for(i; i < arr.length; i++){
       if(i !== 0 || seed){
       combine = func(combine, arr[i], i);
       }
     }
     */
    _.each(arr, function(val, ind, coll) {
        if (ind !== 0 || seed) {
            combine = func(combine, val, ind);
        }
...
_.extend = (...objects) => { 
    ...
    _.each(objects, (obj, key, objects) => {
       if( key === 0 ) return;
